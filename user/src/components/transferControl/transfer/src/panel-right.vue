...
<template>
    <div class="el-transfer-panel">
        <p class="el-transfer-panel__header">
            {{ $t('controlDetail.hasSelected') }}({{ rightChecked.length }})
            <el-button type="text" @click="clearAll">{{ $t('controlDetail.removeAll') }}</el-button>
        </p>
        <div class="el-transfer-panel__body ul-box data-infinite-wrapper">
            <ul infinite-wrapper style="overflow-y: auto"  v-show="rightChecked.length > 0">
                <li v-for="(item,index) in option" :key="item.id" :class="[iconSkinField]">
                    {{item.name}}
