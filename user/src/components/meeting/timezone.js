...
function getTimezoneTimeToInteger(date,utcOffset,ctrl) {
    
    date = checkDate(date);
    if(!ctrl){
        date = new Date(new Date(date).valueOf() + 5*60*1000);
    }
      //延期五分钟
    date = getTimezone(date.valueOf(),utcOffset);
    let H = date.format("HH");
...
function getTomorrowDate(date,utcOffset) {
    date = checkDate(date);
    let tomorrowTimestamp = new Date(date).valueOf() + 24*60*60*1000;
    return getTimezoneDate(tomorrowTimestamp,utcOffset)
}

function getTimezoneDateToInteger(date,utcOffset) {
    
    return getTimezoneTimeToInteger(date,utcOffset,true) === "00:00"
        ?getTomorrowDate(getTimezoneDate(date,utcOffset),utcOffset)
        :getTimezoneDate(date,utcOffset)
}
