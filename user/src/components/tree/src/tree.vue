    ...
    <div class="yl-tree">
        <div class="tree-top">
            <div class="edit-bar" v-if="edit">
                ...
                <el-button type="text" @click="setPrevNode" :disabled="store.disabledPrev" title="上移">
                    <i class="icon iconfont icon-shangyi"></i>
                </el-button>
                <el-button type="text" @click="setNextNode" :disabled="store.disabledNext" title="下移">
                    <i class="icon iconfont icon-xiayi"></i>
                </el-button>
                <el-button type="text" @click="onNodeEdit" :disabled="store.disabledEdit" title="编辑">
                    <i class="icon iconfont icon-bianji"></i>
                </el-button>
                <el-button type="text" @click="onNodeDelete" :disabled="store.disabledDelete" title="删除">
                    <i class="icon iconfont icon-shanchu"></i>
                </el-button>
            </div>
            <div class="search-bar data-infinite-wrapper" v-show="showSearch">
                <el-input :placeholder="$t('common.placeholder.search')" v-model="searchValue" icon="search" @blur="onSearchBlur"></el-input>
                <div class="search " v-show="searchValue&&filterNodes.length>0" infinite-wrapper>
                    <ul>
                        <li v-for="(item,index) in  option" :key="index" @click="searchClick(item)">
        ...
        <div class="tree-down">
            <div class="tree-box el-tree__empty-block" v-if="emptyFilter&&searchValue">
                <p class="empty-filter">
                    {{$t('common.notFound.msg',{displayText:searchValue})}}
                    <a @click="searchValue =''">{{$t('common.clickBack')}}</a>
                </p>
            </div>
...
//            this.store.checkedKeys = newVal;
    ...
    methods: {
        ...
        updateNode(treeNode, checkTypeFlag) {
            this.store.updateNode(treeNode, checkTypeFlag);
        },
        ...
        _filterFun(node) {
            ...
            this.filterNodeKey.forEach(key => {
            	if(node[key]!==null){
                    if (node[key].includes(this.searchValue)) _exits = true;
                }
