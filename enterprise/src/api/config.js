...
const plugin = _Vue => {
    ...
    axios.defaults.baseURL = '/enterprise';
                ...
                switch (_alarmType) {
                    case '':
                        if (error.errorCode == 401){
                            auth.logout();
                            router.replace({ path: '/enterprise/login', query: { code: 401,msg: 'expired' } });
                        }
...
const errorMessage = (type, msg, code) => {
    ...
    if(code !='403'){
        Vue.prototype.$message({
            showClose: true,
            message: 'Internal Server Error' === msg ? t('net.error') : (t(msg)||t('net.error')),
            type: type,
            onClose: () => {
...
const errorDispose = (code) => {
    ...
    if (code == 401) {
        ...
        router.replace({ path: '/enterprise/error', query: { code: code } });
    }
}
