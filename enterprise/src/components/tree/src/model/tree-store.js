...
export default class TreeStore {
    ...
    constructor(options) {
        ...
        this.setting.view = {
            ...
            addDiyDom: (treeId, treeNode) => {
                ...
                if (treeNode.level >= 1) {
                    let spaceStr = `<span  id="${treeNode.tId}_space_switch" style="display: inline-block;width:${(spaceWidth * treeNode.level + (7 * (treeNode.level - 1)))}px"></span>`;// '<span  style=\'display: inline-block;width:' + (spaceWidth * treeNode.level + (7 * (treeNode.level - 1))) + 'px\'></span>';
    ...
    _getDataIconSource() {
        ...
        // try {
        //     if (this.showIcon) {
        //         let _icon = require(`../icon/${this.icon}.png`);
        //         this.iconName = `ztree-${this.icon}`;
        //         let _cssText = `.${this.iconName}_ico_docu { background:url(${_icon})!important;margin:4px 0px 4px 6px!important;} `;
        //         document.styleSheets[0].insertRule(_cssText, 0);
        //         this.data.filter(x => !x.isParent).forEach(d => {
        //             d.iconSkin = this.iconName;
        //         });
        //     }
        // } catch (e) {
        //     console.error('图标错误', e);
        // }
        // return this.data;
    }
     ...
     * 设置选中的Keys
     * */
    setCheckedKeys(newVal, isExpand) {
        let { zTree } = this;
    ...
    updateNode(treeNode, checkTypeFlag,newNode) {
        let {zTree} = this;
        let _targetNode = treeNode;
        if (!(treeNode instanceof Object)) {
            _targetNode = zTree.getNodeByParam('id', treeNode, null);
            Object.assign(_targetNode,newNode);
        }
        zTree.updateNode(_targetNode, checkTypeFlag);
        this.updateStore();
    }
    ...
    moveNode(targetNode, treeNode, moveType, isSilent) {
        ...
        _treeNode.level = _targetNode.level+1;
        const spaceWidth = 16;
        let  width = spaceWidth * _treeNode.level + (7 * (_treeNode.level - 1));
        $('#' + _treeNode.tId + '_space_switch').css('width', width+'px');
    ...
    updateStore() {
        ...
        if (this.edit) {
            ...
            this.disabledDelete = this.currentNode === null || this.currentNode.parentId === null || (this.currentNode.children && this.currentNode.children.length > 0)||(this.currentNode.type === this.type&&this.type==='VMR'&&this.currentNode.innerType !=='cloudPort');
            // (this.currentNode.children && this.currentNode.children.every(x => x.type !== this.type) || false); 除了云端口以外的都可以删除
        }
    }
