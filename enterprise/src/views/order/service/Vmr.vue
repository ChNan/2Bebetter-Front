    ...
    <div class="s-service">
        <div class="m-info">
            <div class="u-ct">
                ...
                <div class="u-info">
                    <p class="u-tt f-fw">{{$t('service.vmr.order')}}</p>
                    <div class="u-server">
                        <ul>
                            <li>
                            ...
                            <li v-for="(val, key) in info.terminal" :key="val">
                                <span class="f-fw">{{key}} {{$t('service.fang')}}：</span>
                    ...
                    <div class="u-server">
                        <ul>
                            <li>
                                <span class="f-fw">• {{$t('service.cloud')}}：</span>
                            </li>
                            <li v-if="!info.cloud">
                                <span class="f-fw"> {{$t('service.noApply')}}</span>
                            </li>
                            <li v-for="(val, key) in info.cloud" :key="val">
                                <span class="f-fw">{{key=='0+0'||key=='0' ? 'N+N':key}} {{$t('service.fang')}}：</span>
                                <span class="f-fc">{{val}} {{$t('service.one')}}</span>
                            </li>
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane :label="$t('service.terminal')" name="first">
                  <div v-if="firstPage.data && firstPage.data.length > 0">
                    ...
                    <div class="table-bar">
                        <div class="u-pagination">
                            <el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1" :current-page="firstPage.currentPage" :page-sizes="[10, 20, 50, 100]" :page-size="firstPage.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="tableData.terminalVmrItems.length">
                            </el-pagination>
                        </div>
                    </div>
                </div>
            ...
            <el-tab-pane :label="$t('service.cloud')" name="second">
               <div v-if="secondPage.data && secondPage.data.length > 0">
                    <el-table class="m-table" ref="table" :data="secondPage.data" stripe @sort-change="sort">
                        <el-table-column prop="createTime" :label="$t('service.createTime')" min-width="160" sortable="custom">
                         ...
                         <el-table-column prop="amount" :label="$t('service.amount')" min-width="100">
                        </el-table-column>
                        <el-table-column prop="limit" :label="$t('service.capacity')" min-width="160">
                        </el-table-column>
                        <el-table-column prop="deadline" :label="$t('service.deadline')"  min-width="200">
                        </el-table-column>
                        <el-table-column prop="endDateStamp" :label="$t('service.validity')" min-width="230" sortable="custom">
                       ...
                       <el-table-column :label="$t('service.desc')" min-width="220" show-overflow-tooltip>
                            <template scope="scope">
                                <span>{{$t('service.vmr.desc', {amount:scope.row.amount, limit: scope.row.limit})}}</span>
                            </template>
                        </el-table-column>
                    </el-table>
                    <div class="table-bar">
                        <div class="u-pagination">
                            <el-pagination @size-change="handleSizeChange2" @current-change="handleCurrentChange2" :current-page="secondPage.currentPage" :page-sizes="[10, 20, 50, 100]" :page-size="secondPage.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="tableData.cloudVmrItems.length">
                            </el-pagination>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    created() {
        ...
        this.tableData.cloudVmrItems.sort(function(x, y) { return x.createTime < y.createTime ? 1 : -1 })
        this.pageData1();
        this.pageData2();
    },
    methods: {
        ...
        pageData1() {
            this.firstPage.data = this.tableData.terminalVmrItems.slice((this.firstPage.currentPage-1) * this.firstPage.pageSize , this.firstPage.currentPage * this.firstPage.pageSize)
        },
        ...
        pageData2() {
            this.secondPage.data = this.tableData.cloudVmrItems.slice((this.secondPage.currentPage-1) * this.secondPage.pageSize , this.secondPage.currentPage * this.secondPage.pageSize)
        },
        sort(a) {
            let type = null;
            this.activeName == 'first' ? type = "terminalVmrItems" : type = "cloudVmrItems"
            this.onsort(a, type)
        }
    },
    components: {
