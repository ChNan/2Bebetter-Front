    ...
    <div class="s-service">
        <div class="m-info">
            <div class="u-ct">
                ...
                <div class="u-info">
                    <p class="u-tt f-fw">{{$t('service.basic.order')}}</p>
                    <div class="u-server">
                        <ul>
                            <li>
                                <span class="f-fw">• {{$t('service.basic')}}</span>
                            </li>
                        </ul>
                    </div>
                    <div class="u-server">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane :label="$t('service.basic')" name="first">
                <div v-if="firstPage.data && firstPage.data.length > 0">
                    <el-table class="m-table" ref="table" :data="firstPage.data" stripe @sort-change="sort">
                        <el-table-column prop="createTime" :label="$t('service.createTime')" min-width="130" sortable="custom">
                            <template scope="scope">
                                <span>{{dateFormat(scope.row.createDate)}}</span>
                            </template>
                        </el-table-column>
                        <el-table-column prop="deadline" :label="$t('service.deadline')" min-width="160">
                        </el-table-column>
                        ...
                        <el-table-column :label="$t('service.desc')" min-width="240" show-overflow-tooltip>
                            <template scope="scope">
                                <span>{{$t('service.basic.desc')}}</span>
                            </template>
                        </el-table-column>
                    </el-table>
                    <div class="table-bar">
                        <div class="u-pagination">
                            <el-pagination @size-change="handleSizeChange1" @current-change="handleCurrentChange1" :current-page="firstPage.currentPage" :page-sizes="[10, 20, 50, 100]" :page-size="firstPage.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="tableData.basicItems.length">
                            </el-pagination>
                        </div>
                    </div>
                </div>
            ...
            <el-tab-pane :label="$t('service.terminal')" name="second">
                <div v-if="secondPage.data && secondPage.data.length > 0">
                    <el-table class="m-table" ref="table" :data="secondPage.data" stripe @sort-change="sort">
                        ...
                        <el-table-column prop="name" :label="$t('service.capacity')" min-width="160">
                            <template scope="scope">
                        ...
                        <el-table-column prop="address" :label="$t('service.desc')" min-width="220" show-overflow-tooltip>
                            <template scope="scope">
...
export default {
    ...
    created() {
        this.tableData.basicItems.sort(function(x, y) { return x.createTime < y.createTime ? 1 : -1 })
    ...
    methods: {
        ...
        pageData1() {
            this.firstPage.data = this.tableData.basicItems.slice((this.firstPage.currentPage-1) * this.firstPage.pageSize , this.firstPage.currentPage * this.firstPage.pageSize )
        },
        ...
        sort(a) {
            let type = null;
            this.activeName == 'first' ? type = "basicItems" : type = "terminalVmrItems"
            this.onsort(a, type)
        }
    },
    components: {
